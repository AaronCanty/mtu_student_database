package ie.mtu.cs.oop.ass1.controller;
...
import ie.mtu.cs.oop.ass1.model.Student;
import ie.mtu.cs.oop.ass1.view.View;
import ie.mtu.cs.oop.ass1.model.Module;
	...
	*/
	...
	public void loadModulesFile() {
		...
		try (BufferedReader bufferRead = new BufferedReader(new FileReader(module_file_name))) {
			...
			for (int i = 0; i < throwAwayArray.size(); i += 13) // Reads file size by line until the end of the module/grades
															// are finished

			/*
			 * The reason why the repeat step in the for loop is set to 13, is because it
			 * will go through each element of the arraylist and print them out in order.
			 * Seeing as the first element is student which only comes up once, the second
			 * element is module name, and third grade, if you add them up you get to 13 -
			 * the allowed amount,
			 */
			{
				studentID = throwAwayArray.get(i); // student is the first in the index
				
				moduleName1 = throwAwayArray.get(i + 1); // module is the second
				moduleGrade1 = Integer.parseInt(throwAwayArray.get(i + 2));// grade is the third
